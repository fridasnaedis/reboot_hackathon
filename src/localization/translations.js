module.exports = {
  is: {
    defaultText: {
      loading: 'Hlutir eru í vinnslu!',
    },
    errors: {
      name: 'Nafn verður að vera allaveganna 5 stafir',
      email: 'Ekki gilt netfang',
    },
    navbar: {
      left: 'reBoot',
      right: 'Text hægri',
    },
    home: {
      banner: 'Borði. Tölvusnápun eftir:',
      intro: 'Stórt Reboot Hack merki hér',
      info: {
        first: 'Hvernig hugmyndin kom upp',
        second: 'Hvað er',
        third:
          'um reboot hack: afhverju nafnið og þá hugmyndin á bak við Reboot',
      },
      detail: {
        first: {
          header:
            'Aðlaðandi texti um Ísland, reykjavík og sömuleiðis háskóla íslands',
          main:
            'Um háskóla íslands að hann sé að verða framsæknari og vill gefa nemendum öðruvísi tækifæri til að læra - einn af stærstu samstarfs og styrktaraðilum. Honum að þakka að við getum haldið þetta. Tala jafnvel aðeins um VON.',
        },
        second: {
          header: 'Hackathon?',
          main:
            'Hakkaþon snýst ekki um að “hakka” heldur um að skapa. Hægt er að hugsa um hakkaþon sem uppfinningar maraþon þar sem þáttakendur fá rými til þess að læra, byggja og skapa. ',
        },
      },
      register: {
        name: 'Nafn',
        email: 'Netfang',
        interest: 'Áhugamál',
        submit: 'Skrá',
        topText: 'Skráðu þig að neðan til þess að taka þátt',
        thanks: 'Takk fyrir skráninguna',
        fail: 'Eitthvað fór úrskeiðis, prófið aftur seinna',
        interestFields: [
          {
            key: 1,
            value: 'Skýjalausnir',
          },
          {
            key: 2,
            value: 'Grafísk Hönnun',
          },
          {
            key: 4,
            value: 'Vefforritun',
          },
        ],
      },
    },
    card: 'mynd hér',
    sponsors: {
      content: 'ssjsaui',
      header: 'Verðflokkar fyrir styrktar- og samstarfsaðila',
      table: {
        r1: 'Styrktaraðila titill',
        r2: 'Merki fyrirtækis á öllu markaðs/kynningarefni',
        r3: 'Útgefið efni varðandi ráðningar á staðnum',
      },
    },
  },
  en: {
    defaultText: {
      loading: 'Working!',
    },
    errors: {
      name: 'Name must be atleast 5 characters',
      email: 'Not a valid email address',
    },
    navbar: {
      left: 'reBoot',
      right: 'Text on right',
    },
    home: {
      banner: 'Banner. Hack in:',
      intro: 'Big Reboot Hack logo here',
      info: {
        first: 'How did the idea come up',
        second: 'What is',
        third: 'About reboot hack, the name and shit',
      },
      detail: {
        first: {
          header: 'Why have a hackathon',
          main:
            'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec aliquet ex justo, in facilisis lacus consequat at. Lorem ipsum dolor',
        },
        second: {
          header: 'Who is this for',
          main:
            'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec aliquet ex justo, in facilisis lacus consequat at. Lorem ipsum dolor',
        },
      },
      register: {
        name: 'Name',
        email: 'Email',
        interest: 'Interests',
        submit: 'Register',
        topText: 'Sign up bellow to join the fun',
        thanks: 'Thanks for signing up',
        fail: 'Something went wrong, please try again later',
        interestFields: [
          {
            key: 1,
            value: 'Cloud based solutions',
          },
          {
            key: 2,
            value: 'Graphic design',
          },
          {
            key: 4,
            value: 'Web application',
          },
        ],
      },
    },
    card: 'image here',
    sponsors: {
      content: 'some here more',
      header: 'Lorem',
      table: {
        r1: 'Lorem',
        r2: 'Lorem Lorem',
        r3: 'Lorem Lorem Lorem',
      },
    },
  },
};
